...
test.describe('Portfolio Page - About Section', () => {
  ...
  test('Verify header and basic elements are visible', async ({ page }) => {
    await expect(page.getByRole('heading', { name: 'Gaurav Thorat' })).toBeVisible();
    await expect(page.getByText('Frontend Developer', { exact: true })).toBeVisible();
    await expect(page.getByText('Technical Lead', { exact: true })).toBeVisible();
    await expect(page.getByRole('img', { name: 'Gaurav Thorat - Frontend' })).toBeVisible();
  });
...
test.describe('Portfolio Page - Navigation and Content', () => {
  ...
  test('Navigate to Projects section', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Executed Projects' })).toBeVisible();
  });
  ...
  test('Navigate to CV section', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'My detailed professional' })).toBeVisible();
  });
});
